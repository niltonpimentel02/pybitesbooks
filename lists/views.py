...
class UserListListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            ...
            num_user_lists = UserList.objects.filter(user=self.request.user).count()
...
class UserListDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        obj = self.get_object()
        user_books = UserBook.objects.select_related("book", "user").order_by(
            "book__title"
        )
        ...
        user_books = user_books.filter(booklists__id=obj.id)
        ...
        books_by_category_sorted = sorted(books_by_category.items())
...
class UserListCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if user_lists.filter(user=self.request.user).count() > max_num_user_lists:
            form.add_error("name", f"You can have {max_num_user_lists} lists at most")
            return self.form_invalid(form)
