...
def test_user_profile_page_stars(client, user, user_fav_books, snippet):
    ...
    html = response.content.decode()
    assert (
        f'<input class="js-favorite" title="favorite"'
        f' type="checkbox" bookid={snippet}'
    ) in html
