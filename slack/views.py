...
from api.views import get_users, get_user_last_book, get_random_book
...
def _create_user_output(user_books):
    ...
    for user, last_book in sorted(users, key=lambda x: x[1].completed, reverse=True):
...
def _get_attachment(msg, book=None):
    if book is None:
        return {"text": msg, "color": "#3AA3E3"}
    else:
        return {
            "title": book["title"],
            "title_link": book["url"],
            "image_url": BOOK_THUMB.format(
                bookid=book["bookid"], imagesize=book["imagesize"]
            ),
            "text": msg,
            "color": "#3AA3E3",
        }
